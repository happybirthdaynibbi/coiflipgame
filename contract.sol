...
contract CoinFlip is Ownable {
    ...
    address public _houseFeeReceiver = address(0xf82A3B66c62e8b7A5968fF0856851E2e15B83841);
    uint8 public _houseFeePercentage = 50; 

    address public _devFeeReceiver = address(0x17e8CD8151fCd52664Da22C2B7386A6EB4B18142);
    ...
    function claimWinnings(address token) external {
        ...
        if (token != address(0x0)) {
            ...
            (bool successDev, ) = _devFeeReceiver.call{value: feeToDev}("");
            (bool successHouse, ) = _houseFeeReceiver.call{value: feeToHouse}("");
            require(successDev, "Dev fee transfer failed");
            require(successHouse, "House fee transfer failed");

            if (feeToReferrer > 0) {
                (bool successReferrer, ) = referrer.call{value: feeToReferrer}("");
                require(successReferrer, "Referrer fee transfer failed");
            }

            (bool successUser, ) = _msgSender().call{value: winningsToUser}("");
            require(successUser, "User winnings transfer failed");
        }
